...
static int load_oci_credentials(struct flb_oci_logan *ctx)
{
    ...
    while (line != NULL) {
        ...
        if (found_profile) {
            ...
            char *orig_key = mk_string_copy_substr(line, 0, eq_pos);
...
flb_sds_t extract_region(const char *response)
{
    ...
    region[len] = '\0';
    const char *long_name = long_region_name(region);
    const char *region_value = long_name ? long_name : region;
...
bool extract_tenancy_ocid(struct flb_oci_logan *ctx, const char *cert_pem)
{
    ...
    for (i = 0; i < entry_count; i++) {
        ...
        if (OBJ_obj2nid(obj) == NID_organizationalUnitName) {
            ...
            const unsigned char *ou = ASN1_STRING_get0_data(data);
            int ou_len = ASN1_STRING_length(data);
            const char *prefix = "opc-tenant:ocid1.tenancy";
            size_t prefix_len = strlen(prefix);

            if (ou && ou_len > (int) prefix_len && memcmp(ou, prefix, prefix_len) == 0) {
                const unsigned char *colon = memchr(ou, ':', ou_len);
                if (colon && (colon + 1) < (ou + ou_len)) {
                    size_t ocid_len = (ou + ou_len) - (colon + 1);
                    tenancy_ocid = flb_sds_create_len((const char *) (colon + 1), ocid_len);
                    break;
                }
            }
        }
...
flb_sds_t create_federation_payload(struct flb_oci_logan *ctx)
{
    ...
    flb_sds_t intermediate_certs = NULL;
                                                ...
                                                struct flb_config *config)
{
    ...
    if (strcmp(ctx->auth_type, "instance_principal") == 0) {
        ...
        ctx->imds.session_pubkey =
            extract_public_key_pem(ctx->session_key_pair);
        ctx->imds.session_privkey =
            extract_private_key_pem(ctx->session_key_pair);
        ...
        flb_sds_t response =
            sign_and_send_federation_request(ctx, json_payload);
        if (!response) {
            flb_plg_error(ctx->ins,
