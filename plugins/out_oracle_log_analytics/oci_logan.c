...
static const char* oci_supported_timezones[] = {
    ...
    "asia/riyadh", "asia/riyadh87", "asia/riyadh88", "asia/riyadh89", "asia/saigon",
    ...
    "mideast/riyadh88", "mideast/riyadh89", "mst", "mst7mdt", "navajo", "nz", "nz-chat",
    ...
    "prc", "pst", "pst8pdt", "rok", "singapore", "systemv/ast4", "systemv/ast4adt",
    "systemv/cst6", "systemv/cst6cdt", "systemv/est5", "systemv/est5edt", "systemv/hst10",
    "systemv/mst7", "systemv/mst7mdt", "systemv/pst8", "systemv/pst8pdt", "systemv/yst9",
    "systemv/yst9ydt", "turkey", "uct", "universal", "us/alaska", "us/aleutian",
...
static int init_oci_timezone_hash(void) {
    ...
    oci_timezone_hash = flb_hash_table_create(FLB_HASH_TABLE_EVICT_NONE, 1024, -1);
...
static int is_oci_supported_timezone(const char *timezone) {
    void *out_buf;
    size_t out_size;
    int ret;
    char *lower_tz;
    ...
    fprintf(stderr, "is_oci_supported_timezone::init_oci_timezone_hash->[%d]\n",ret_init_oci_timezone_hash);
    fflush(stderr);
    if (ret_init_oci_timezone_hash != 0) {
        return 0;
    }
    lower_tz = strdup(timezone);
    if (!lower_tz) {
        return 0;
    }
    ...
    fflush(stderr);
    ret = flb_hash_table_get(oci_timezone_hash, lower_tz, strlen(lower_tz), 
    &out_buf, &out_size);
...
static int is_valid_timezone(const char *timezone) {
    ...
    fflush(stderr);
    // add checker for timezone like 00:10....
    if (is_oci_supported_timezone(timezone)) {
        fprintf(stderr, "timezone %s is valid", timezone);
        fflush(stderr);
        return 1;
    }
    return 0;
